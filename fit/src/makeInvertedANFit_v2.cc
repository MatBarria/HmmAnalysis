...
RooWorkspace *makeMCFit(TTree *tree, float forceSigma, bool constrainMu,
                        float forceMu, TString fit_variable_name) {
    ...
    RooRealVar mgg("diMuon_mass", "m_{#mu#mu}", 103, 160, "GeV");
    mgg.setBins(38);
...
RooWorkspace *makeInvertedANFit(TTree *tree, float forceSigma = -1,
    ...
    RooRealVar mgg(mggName, "m_{#mu#mu}", 103, 160, "GeV");

    mgg.setBins(38);

    mgg.setRange("sideband_low", 103, 120);
    mgg.setRange("sideband_high", 130, 160);
    mgg.setRange("Full", 103, 160);
    //  TString
    ...
    // RooDataSet* blind_data = (RooDataSet*)data.reduce("mgg < 121.0 ||
    // mgg>130.0");
