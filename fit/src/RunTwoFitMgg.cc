...
#include "../include/FitConstants.h"
#include "RunTwoFitMgg.hh"

RooWorkspace *DoubleGausFit(TTree *tree, float forceSigma, bool sameMu,
                            float forceMu, TString mggName) {
    RooWorkspace *ws = new RooWorkspace("ws", "");
    RooRealVar mgg(mggName, "m_{#gamma#gamma}", 106, 160, "GeV");
    mgg.setBins(10);
    // mgg.setRange( "signal", 110, 140. );
    mgg.setRange("signal", 106, 160.);
    ...
    */
    ...
    // ws->pdf( tag )->fitTo( data, RooFit::Strategy(0), RooFit::Extended( kTRUE
    // ), RooFit::Range("signal") );
    RooFitResult *sres =
        ws->pdf(tag)->fitTo(data, RooFit::Strategy(2), RooFit::Extended(kTRUE),
                            RooFit::Save(kTRUE), RooFit::Range("signal"));
    ...
    data.plotOn(frame);
    ws->pdf(tag)->plotOn(frame, RooFit::LineColor(kBlue), RooFit::Range("Full"),
...
//RooWorkspace *DoubleCBFit(TTree *tree, RooWorkspace *ws, TString mggName, float muCB,
void DoubleCBFit(TTree *tree, RooWorkspace *ws, TString mggName, float muCB,
                          float sigmaCB) {

    //RooWorkspace *ws = new RooWorkspace("ws", "");
    RooRealVar mgg(mggName, "m_{#mu#mu}", SIDEBAND_LOW_BOTTOM_LIM,
                   SIDEBAND_HIGH_UPPER_LIM, "GeV");
    mgg.setBins(40);
    mgg.setRange("Full", SIDEBAND_LOW_BOTTOM_LIM, SIDEBAND_HIGH_UPPER_LIM);
    // mgg.setRange("signal",110,140);

    RooRealVar w("weight", "w", -100, 100);
    RooDataSet data("data", "", RooArgSet(mgg, w), RooFit::WeightVar(w),
                    RooFit::Import(*tree));
    // RooDataSet data( "data", "", RooArgSet(mgg), RooFit::Import( *tree ) );
    double npoints = data.sumEntries("", "event_weight");

    //---------------------------------
    // C r e a t e  Double Crystall Ball
    //---------------------------------
    TString tag = MakeDoubleCB("Sig", mgg, *ws);
    ws->var(tag + "_Ns")->setVal((double)npoints);
    // ws->pdf( tag
    // )->fitTo(data,RooFit::Strategy(0),RooFit::Extended(kTRUE),RooFit::Range("signal"));
    RooFitResult *sres =
        ws->pdf(tag)->fitTo(data, RooFit::Strategy(2), RooFit::Extended(kTRUE),
                            RooFit::Save(kTRUE), RooFit::Range("Full"));
...
RooWorkspace *MakeSideBandFit(TTree *tree, float forceSigma, bool constrainMu,
                              float forceMu, TString mggName) {
    RooWorkspace *ws = new RooWorkspace("ws", "");
    ...
    // RooDataSet data( "data", "", RooArgSet(mgg, w), RooFit::WeightVar(w),
    // RooFit::Import(*tree) ); Sideband Fit
    RooDataSet data("data", "", RooArgSet(mgg), RooFit::Import(*tree));
    ws->pdf(tag3)->fitTo(data, RooFit::Strategy(0), RooFit::Extended(kTRUE),
                         RooFit::Range("low,high"));
    RooFitResult *bres =
        ws->pdf(tag3)->fitTo(data, RooFit::Strategy(2), RooFit::Extended(kTRUE),
                             RooFit::Save(kTRUE), RooFit::Range("low,high"));
    // Full Fit
    // ws->pdf( tag3 )->fitTo( data, RooFit::Strategy(0),
    // RooFit::Extended(kTRUE), RooFit::Range("Full") ); RooFitResult* bres =
    // ws->pdf( tag3 )->fitTo( data, RooFit::Strategy(2), RooFit::Save(kTRUE),
    // RooFit::Extended(kTRUE), RooFit::Range("Full") );

    mgg.setRange("sregion", 122.04, 128.96);
    RooAbsReal *sint = ws->pdf(tag3)->createIntegral(mgg, RooFit::NormSet(mgg),
                                                     RooFit::Range("sregion"));
    RooAbsReal *Nfit =
        ws->pdf(tag3)->createIntegral(mgg, RooFit::Range("Full"));
    std::cout << sint->getVal() << std::endl;
    // float N_sideband = data.sumEntries(Form("(mgg>%0.2f && mgg <120) ||
    // (mgg>131 && mgg<%0.2f)",103.,160.));
    /*
    ...
    */
    ...
    data.plotOn(fmgg);
    std::cout << "here 1" << std::endl;
    ws->pdf(tag3)->plotOn(fmgg, RooFit::LineColor(kRed), RooFit::Range("Full"),
                          RooFit::NormRange("Full"));
    ws->pdf(tag3)->plotOn(fmgg, RooFit::LineColor(kBlue),
                          RooFit::LineStyle(kDashed), RooFit::Range("low,high"),
                          RooFit::NormRange("low,high"));
    std::cout << "here 1" << std::endl;
    // ws->pdf( tag3 )->plotOn(fmgg,RooFit::LineColor(kBlue),
    // RooFit::LineStyle(kDashed),
    // RooFit::Range("low,high"),RooFit::NormRange("Full"));
    std::cout << "here 1" << std::endl;
    ...
    std::cout << "here 1" << std::endl;
    ws->pdf(tag3)->plotOn(pdfFrame, RooFit::LineColor(kViolet),
                          RooFit::Range("Full"), RooFit::NormRange("Full"));
    std::cout << "here 1" << std::endl;
...
void MakePlot(TTree *tree, RooWorkspace &w, TString pdfName, TString mggName) {
    ...
    RooDataSet data("data", "", RooArgSet(mgg), RooFit::Import(*tree));
    // fullFit
    // w.pdf( tag3 )->fitTo( data, RooFit::Strategy(0), RooFit::Extended(kTRUE),
    // RooFit::Range("Full") ); RooFitResult* bres = w.pdf( tag3 )->fitTo( data,
    // RooFit::Strategy(2), RooFit::Save(kTRUE), RooFit::Extended(kTRUE),
    // RooFit::Range("Full") ); sidebandFit
    w.pdf(tag3)->fitTo(data, RooFit::Strategy(0), RooFit::Extended(kTRUE),
                       RooFit::Range("low_v2,high_v2"));
    RooFitResult *bres = w.pdf(tag3)->fitTo(
        data, RooFit::Strategy(2), RooFit::Save(kTRUE), RooFit::Extended(kTRUE),
        RooFit::Range("low_v2,high_v2"));

    RooPlot *fmgg = mgg.frame();
    data.plotOn(fmgg);
    // w.pdf( pdfName )->plotOn( fmgg, RooFit::LineColor(kBlue),
    // RooFit::Range("low,high"), RooFit::NormRange("low,high") );

    // w.pdf( pdfName )->plotOn( fmgg, RooFit::LineColor(kBlue),
    // RooFit::LineStyle(kDashed), RooFit::Range("Full"),
    // RooFit::NormRange("low_v2,high_v2") );
    w.pdf(pdfName)->plotOn(fmgg, RooFit::LineColor(kGreen),
                           RooFit::Range("Full"),
                           RooFit::NormRange("low,high"));
    // w.pdf( tag3 )->plotOn( fmgg, RooFit::LineColor(kRed),
    // RooFit::Range("Full"), RooFit::NormRange("low_v2,high_v2") );
...
double GetIntegral(RooWorkspace &w, TString pdfName, TString mggName) {
    ...
    RooAbsReal *fIntegral2 = NewModel->createIntegral(
        *mgg, RooFit::NormSet(*mgg), RooFit::Range("sig"));
    std::cout << "test Int: " << fIntegral->getVal() << std::endl;
    std::cout << "test Int2: " << fIntegral2->getVal() << std::endl;
    mgg->setMax(400);
    mgg->setMin(100);
    mgg->setRange("sig", 131, 400);
    fIntegral2 = NewModel->createIntegral(*mgg, RooFit::NormSet(*mgg),
                                          RooFit::Range("sig"));
    std::cout << "test Int2': " << fIntegral2->getVal() << std::endl;
    return fIntegral2->getVal();
};

RooWorkspace *MakeSignalBkgFit(TTree *treeData, TTree *treeSignal,
                               TTree *treeSMH, TString mggName) {
    //------------------------------------------------
    ...
    RooDataSet dataSignal("dataSignal", "", RooArgSet(mgg),
                          RooFit::Import(*treeSignal));
    ...
    RooFitResult *sres = ws->pdf(tagSignal)->fitTo(
        dataSignal, RooFit::Strategy(2), RooFit::Extended(kTRUE),
        RooFit::Save(kTRUE), RooFit::Range("full"));
    ...
    RooFitResult *smhres = ws->pdf(tagSMH)->fitTo(
        dataSMH, RooFit::Strategy(2), RooFit::Extended(kTRUE),
        RooFit::Save(kTRUE), RooFit::Range("full"));
    ...
    RooFitResult *bres = ws->pdf(tag_bkg)->fitTo(
        data, RooFit::Strategy(2), RooFit::Extended(kTRUE), RooFit::Save(kTRUE),
        RooFit::Range("low,high"));
    ...
    // RooDataSet* data_toys  = ws->pdf( tag_bkg )->generate(mgg, npoints,
    // kFALSE, kTRUE, "", kTRUE, kFALSE);
    RooAbsData *data_toys =
        ws->pdf(tag_bkg)->generateBinned(mgg, npoints, RooFit::ExpectedData());
    ...
    // RooAddPdf* model = new RooAddPdf( "model", "model", RooArgList( *ws->pdf(
    // tag_signal2 ), *ws->pdf( tag_SMH2 ), *ws->pdf( tag_bkg2 ) ) ) ;
    // RooAddPdf* model = new RooAddPdf( "model", "model", RooArgSet( *ws->pdf(
    // tag_signal2 ), *ws->pdf( tag_bkg2 ) ) ) ; RooAddPdf* model = new
    // RooAddPdf( "model", "model", RooArgSet( *ws->pdf( tag_bkg2 ), *ws->pdf(
    // tag_sg ) ) ) ;
    ...
    RooAddPdf *model = new RooAddPdf(
        "model", "model", RooArgSet(*ws->pdf(tag_bkg2), *ws->pdf(tag_sg)),
        RooArgSet(NbkgModel, NsModel));
    ...
    // RooGaussian SMH_Constraint("SMH_Constraint", "SMH_Constraint",
    // *ws->var("DG_SMH_DGF_Ns"), RooFit::RooConst(0.1), RooFit::RooConst(0.01)
    // ); RooGaussian SMH_Constraint("SMH_Constraint", "SMH_Constraint",
    // *ws->var("DG_SMH_DG_Ns"), HiggsYield, HiggsYieldUn );
    RooRealVar BkgYield("BkgYield", "", npoints);
    RooRealVar BkgYieldUn("BkgYieldUn", "", 0.1 * sqrt(npoints));
    // RooGaussian Bkg_Constraint("Bkg_Constraint", "Bkg_Constraint",
    // *ws->var("BkgModel_singleExp_Nbkg"), BkgYield, BkgYieldUn );
    ...
    RooRealVar HiggsMass("HiggsMass", "", ws->var("Signal_SG_mu")->getVal());
    RooRealVar HiggsMassUn("HiggsMassUn", "",
                           0.05 * ws->var("Signal_SG_mu")->getVal());
    std::cout << "[INFO]: MC measured mass is:  " << HiggsMass.getVal()
              << " +/- " << HiggsMassUn.getVal() << std::endl;
    RooGaussian HiggsMass_Constraint("SMH_Constraint", "SMH_Constraint",
                                     *ws->var("Signal_SG_mu"), HiggsMass,
                                     HiggsMassUn);
    ...
    // F i t   t o   D a t a
    //---------------------
    // RooFitResult* sbres = model->fitTo( data, RooFit::Strategy(2),
    // RooFit::Extended(kTRUE), RooFit::Save(kTRUE),
    // RooFit::ExternalConstraints(SMH_Constraint) ,RooFit::Range("low,high") );

    TH1F *h_1 = new TH1F("h_1", "h_1", 100, 0, 10);
    RooFitResult *sbres;
    // sbres = model->fitTo( *data_toys, RooFit::Strategy(2),
    // RooFit::Extended(kTRUE), RooFit::Save(kTRUE), RooFit::Range("full") );
    // sbres = model->fitTo( data, RooFit::Strategy(2), RooFit::Extended(kTRUE),
    // RooFit::Save(kTRUE), RooFit::Range("full") );
    sbres = model->fitTo(data, RooFit::Strategy(2), RooFit::Extended(kTRUE),
                         RooFit::Save(kTRUE),
                         RooFit::ExternalConstraints(HiggsMass_Constraint),
                         RooFit::Range("full"));
    ...
    // RooFitResult* sbres = model->fitTo( data, RooFit::Strategy(2),
    // RooFit::Extended(kTRUE), RooFit::Save(kTRUE),
    // RooFit::ExternalConstraints(Bkg_Constraint),RooFit::Range("Full") );
    ...
    // RooFit::Extended(kTRUE), RooFit::Range("Full") ); Defininf RooMinimizer
    // Object; RooMinimizer m(*nll); m.setStrategy(2); m.setPrintLevel(3);
    ...
    model->plotOn(fmgg, RooFit::LineColor(kRed), RooFit::Range("Full"),
                  RooFit::NormRange("Full"));
    model->plotOn(fmgg, RooFit::LineColor(kBlue), RooFit::LineStyle(kDashed),
                  RooFit::Range("low,high"), RooFit::NormRange("low,high"));
    ...
    ws->pdf(tagSignal)->plotOn(fmgg2, RooFit::LineColor(kRed),
                               RooFit::Range("Full"),
                               RooFit::NormRange("Full"));
    ws->pdf(tagSignal)->plotOn(
        fmgg2, RooFit::LineColor(kBlue), RooFit::LineStyle(kDashed),
        RooFit::Range("low,high"), RooFit::NormRange("low,high"));
    ...
    ws->pdf(tagSMH)->plotOn(fmgg3, RooFit::LineColor(kRed),
                            RooFit::Range("Full"), RooFit::NormRange("Full"));
    ws->pdf(tagSMH)->plotOn(
        fmgg3, RooFit::LineColor(kBlue), RooFit::LineStyle(kDashed),
        RooFit::Range("low,high"), RooFit::NormRange("low,high"));
    ...
    */
    ...
    // RooPlot* fns = ws->var("DG_Signal_DG_Ns")->frame( RooFit::Range(0, 30,
    // true) ); RooPlot* fns = ws->var("Signal_SG_Ns")->frame(
    // RooFit::Range(-10, 10, true) ); RooPlot* fns = NsModel.frame(
    // RooFit::Range(-10, 10, true) );
    RooPlot *fns =
        NsModel.frame(RooFit::Range(sVal - 5. * sErr, sVal + 5. * sErr, true));
    // RooPlot* fns = ws->var("DG_Signal_DG_Ns")->frame( RooFit::Range(
    // sVal-5.*sErr, sVal+5.*sErr, true) );
    fns->SetMinimum(0);
    fns->SetMaximum(12);
...
RooWorkspace *MakeDataCard(TTree *treeData, TTree *treeSignal, TTree *treeSMH,
                           TString mggName, float SMH_Yield, std::string SMH_CF,
                           float Signal_Yield, std::string Signal_CF,
                           TString binNumber, TString category) {
    std::cout << "entering datacard: " << SMH_Yield << " " << Signal_Yield
              << std::endl;
    ...
    // C r e a t e   s i g n a l  s h a p e from TTree
    //------------------------------------------------
    TString combinedRootFileName =
        "HggRazorWorkspace_bin" + binNumber + ".root";
    ...
    // *ws->var("DG_SMH_DGF_Ns"), RooFit::RooConst(0.1), RooFit::RooConst(0.01)
    // ); RooGaussian SMH_Constraint("SMH_Constraint", "SMH_Constraint",
    // *ws->var(tagSMH+"_Ns"), HiggsYield, HiggsYieldUn );
    std::cout << "pass constraints" << std::endl;
    std::cout << "pass forceSigma" << std::endl;
    ...
    RooPlot *fmgg = mgg.frame();
    data_toys->plotOn(fmgg);
    ws->pdf(tag_bkg)->plotOn(fmgg, RooFit::LineColor(kRed),
                             RooFit::Range("Full"), RooFit::NormRange("Full"));
    ws->pdf(tag_bkg)->plotOn(
        fmgg, RooFit::LineColor(kBlue), RooFit::LineStyle(kDashed),
        RooFit::Range("low,high"), RooFit::NormRange("low,high"));
    ...
    TString combineSMH;
    if (category != "highres")
        combineSMH = MakeFullDoubleGaussNE(
            "SMH_bin" + binNumber, mgg, *combine_ws, true, true,
            category); // add global and cat scale uncertainty
    else
        combineSMH =
            MakeFullDoubleGaussNE("SMH_bin" + binNumber, mgg, *combine_ws,
                                  true); // adding global scale uncertainty only
    ...
    if (category != "highres")
        combineSignal = MakeFullDoubleGaussNE(
            "signal_bin" + binNumber, mgg, *combine_ws, true, true,
            category); // add global and cat scale uncertainty
    else
        combineSignal =
            MakeFullDoubleGaussNE("signal_bin" + binNumber, mgg, *combine_ws,
                                  true); // add global scale uncertainty only
    ...
    //---------
    TString combineBkg =
        MakeSingleExpNE("Bkg_bin" + binNumber, mgg, *combine_ws);
    ...
    ofs << "-------------------------------------------------------------------"
           "---------------------\n";
    ofs << "shapes Bkg\t\tbin" << binNumber << "\t" << combinedRootFileName
        << " combineWS:" << combineBkg << "\n";
    ofs << "shapes SMH\t\tbin" << binNumber << "\t" << combinedRootFileName
        << " combineWS:" << combineSMH << "\n";
    ofs << "shapes signal\t\tbin" << binNumber << "\t" << combinedRootFileName
        << " combineWS:" << combineSignal << "\n";
    ofs << "shapes data_obs\t\tbin" << binNumber << "\t" << combinedRootFileName
        << " combineWS:" << "data_bin" << binNumber << "\n";
    ofs << "-------------------------------------------------------------------"
           "---------------------\n";
    ...
    for (int isys = 0; isys < totalSys; isys++) {
        if (isys == 0) {
            ofs << "SMH_JES\t\t\t\tlnN\t\t-\t\t" << smh_sys.at(isys + 1) << "/"
                << smh_sys.at(isys) << "\t\t-\n";
        } else if (isys == 2) {
            ofs << "SMH_facScale\t\t\tlnN\t\t-\t\t" << smh_sys.at(isys + 1)
                << "/" << smh_sys.at(isys) << "\t\t-\n";
        } else if (isys == 4) {
            ofs << "SMH_renScale\t\t\tlnN\t\t-\t\t" << smh_sys.at(isys + 1)
                << "/" << smh_sys.at(isys) << "\t\t-\n";
        } else if (isys == 6) {
            ofs << "SMH_facRenScale\t\t\tlnN\t\t-\t\t" << smh_sys.at(isys + 1)
                << "/" << smh_sys.at(isys) << "\t\t-\n";
        } else if (isys > 7) {
    ...
    // if ( category != "highres" ) ofs << category <<
    // "_mu_Global\t\t\tparam\t\t 0 1.25\n";
    if (category == "hzbb") {
        ofs << "SMH_btag\t\t\tlnN\t\t-\t\t"
            << "0.961/1.04"
               "\t\t-\n";
    ...
    // "\t\t-\n"; ofs << "BkgNorm_bin" << binNumber << "\t\t\tlnN\t\t-\t\t-\t\t"
    // << BkgNormUn << std::endl;
    ofs.close();
    return ws;
};

RooWorkspace *MakeSignalBkgFit(TTree *tree, float forceSigma, bool constrainMu,
                               float forceMu, TString mggName) {
    ...
    //---------------
    RooAddPdf *model = new RooAddPdf(
        "model", "model", RooArgSet(*ws->pdf(tag_signal2), *ws->pdf(tag_bkg)));
    // RooAddPdf* model2 = new RooAddPdf( "model2", "model2", RooArgSet(
    // *ws->pdf( tag_signal2 ), *ws->pdf( tag_bkg2 ) ) ) ; Fixing gaussian
    // parameter for 2nd model
    ...
    RooGaussian HiggsMassConstraint("HiggsMassConstraint","",
    *ws->var("fullsb_fit_signal_gauss_mu"),HiggsMass,HiggsMassError); std::cout
    << "pass constraints" << std::endl; std::cout << "entering forceSigma" <<
    std::endl; if( forceSigma != -1 ) {
    ...
    */
    ...
    model->fitTo(data, RooFit::Strategy(0), RooFit::Extended(kTRUE),
                 RooFit::Range("Full"));
    RooFitResult *bres =
        model->fitTo(data, RooFit::Strategy(0), RooFit::Extended(kTRUE),
                     RooFit::Save(kTRUE), RooFit::Range("Full"));

    // model2->fitTo( data, RooFit::Strategy(0), RooFit::Extended(kTRUE),
    // RooFit::Range("Full") ); RooFitResult* bres2 = model->fitTo( data,
    // RooFit::Strategy(0), RooFit::Extended(kTRUE), RooFit::Save(kTRUE),
    // RooFit::Range("Full") );
    ...
    */
    ...
    model->fitTo( data, RooFit::Strategy(0), RooFit::Extended(kTRUE),
    RooFit::Range("Full"),
    RooFit::ExternalConstraints(RooArgSet(HiggsMassConstraint)) ); RooFitResult*
    bres = model->fitTo( data, RooFit::Strategy(0), RooFit::Extended(kTRUE),
    RooFit::Save(kTRUE), RooFit::Range("Full"),
    RooFit::ExternalConstraints(RooArgSet(HiggsMassConstraint)) );
    */
    //--------------------------------
    // m o d e l   1   p l o t t i n g
    //--------------------------------
    ...
    model2->plotOn(fmgg2,RooFit::LineColor(kBlue), RooFit::LineStyle(kDashed),
    RooFit::Range("low,high"),RooFit::NormRange("low,high")); fmgg2->SetName(
    "fullsb_fit_frame2" ); ws->import( *model2 ); ws->import( *bres2 );
    ws->import( *fmgg2 );*/
    ...
    //--------------------------------------
    RooPlot *fns =
        ws->var("doubleGaussSB_gauss_Ns")->frame(RooFit::Range(0, 20, true));
    fns->SetMinimum(0);
    fns->SetMaximum(6);
    n2ll.plotOn(fns, RooFit::ShiftToZero(), RooFit::LineColor(kBlue));
    p2ll->plotOn(fns, RooFit::LineColor(kBlack));
...
RooWorkspace *MakeSideBandFitAIC(TTree *tree, float forceSigma,
                                 bool constrainMu, float forceMu,
                                 TString mggName, double &AIC,
                                 TString ffName = "doubleExp") {
    RooWorkspace *ws = new RooWorkspace("ws", "");
    ...
    if (ffName == "doubleExp") {
        ...
        std::cout << "[ERROR]: fit option not recognized. QUITTING PROGRAM"
                  << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    // Sideband Fit
    RooDataSet data("data", "", RooArgSet(mgg), RooFit::Import(*tree));
    // ws->pdf( tag )->fitTo( data, RooFit::Strategy(0),
    // RooFit::Extended(kTRUE), RooFit::Range("low,high") ); RooFitResult* bres
    // = ws->pdf( tag )->fitTo( data, RooFit::Strategy(0),
    // RooFit::Extended(kTRUE), RooFit::Save(kTRUE), RooFit::Range("low,high")
    // );
    RooFitResult *bres =
        ws->pdf(tag)->fitTo(data, RooFit::Strategy(0), RooFit::Extended(kTRUE),
                            RooFit::Save(kTRUE), RooFit::Range("Full"));
    // ws->pdf( tag )->fitTo( data, RooFit::Strategy(0),
    // RooFit::Extended(kTRUE), RooFit::Range("Full") ); RooFitResult* bres =
    // ws->pdf( tag )->fitTo( data, RooFit::Strategy(2),
    // RooFit::Extended(kTRUE), RooFit::Save(kTRUE), RooFit::Range("low,high")
    // );
    ...
    // RooAbsReal* nll = ws->pdf( tag )->createNLL(data,  RooFit::Strategy(2),
    // RooFit::Extended(kTRUE), RooFit::Range("low,high") );
    RooAbsReal *nll = ws->pdf(tag)->createNLL(data, RooFit::Extended(kTRUE),
                                              RooFit::Range("Full"));
    ...
    */
    ...
    RooPlot *fmgg = mgg.frame();
    data.plotOn(fmgg);
    ws->pdf(tag)->plotOn(fmgg, RooFit::LineColor(kRed), RooFit::Range("Full"),
                         RooFit::NormRange("Full"));
    ws->pdf(tag)->plotOn(fmgg, RooFit::LineColor(kBlue),
                         RooFit::LineStyle(kDashed), RooFit::Range("low,high"),
                         RooFit::NormRange("low,high"));
    ...
    RooPlot *pdfFrame = mgg.frame();
    ws->pdf(tag)->plotOn(pdfFrame, RooFit::LineColor(kViolet),
                         RooFit::Range("Full"), RooFit::NormRange("Full"));
...
RooWorkspace *DoBiasTest(TTree *tree, TString mggName, TString f1, TString f2,
    ...
    */
    ...
    // ws->pdf( tag1 )->fitTo( data, RooFit::Strategy(0),
    // RooFit::Extended(kTRUE), RooFit::Range("low,high") ); RooFitResult* bres
    // = ws->pdf( tag1 )->fitTo( data, RooFit::Strategy(2),
    // RooFit::Extended(kTRUE), RooFit::Save(kTRUE), RooFit::Range("low,high")
    // ); FullFit
    RooFitResult *bres = ws->pdf(tag1)->fitTo(
        data, RooFit::Strategy(0), RooFit::Save(kTRUE), RooFit::Extended(kTRUE),
        RooFit::Range("low,high"));
    // RooFitResult* bres = ws->pdf( tag1 )->fitTo( data, RooFit::Strategy(0),
    // RooFit::Extended(kTRUE), RooFit::Save(kTRUE), RooFit::Range("low,high")
    // );
    ...
    ws->pdf(tag1)->plotOn(f1_frame, RooFit::LineColor(kRed),
                          RooFit::Range("low,high"),
                          RooFit::NormRange("low,high"));
    ws->import(*f1_frame);
    RooAbsReal *f1Integral = ws->pdf(tag1)->createIntegral(
        mgg, RooFit::NormSet(mgg), RooFit::Range("sig"));
    ...
    //-----------------------
    ws->pdf(tag2p)->fitTo(data, RooFit::Strategy(0), RooFit::Extended(kTRUE),
                          RooFit::Range("low,high"));
    RooPlot *pFrame = mgg.frame();
    pFrame->SetName("pFitFrame");
    data.plotOn(pFrame);
    ws->pdf(tag2p)->plotOn(pFrame, RooFit::LineColor(kBlue),
                           RooFit::Range("low,high"),
                           RooFit::NormRange("low,high"));
    ws->pdf(tag2p)->plotOn(pFrame, RooFit::LineStyle(kDashed),
                           RooFit::LineColor(kRed), RooFit::Range("Full"),
                           RooFit::NormRange("low,high"));
    ws->import(*pFrame);
    ...
    for (int i = 0; i < ntoys; i++) {
        ...
        // ws->pdf( tag2 )->fitTo( *data_toys, RooFit::Strategy(0),
        // RooFit::Extended(kTRUE), RooFit::Range("low,high") ); RooFitResult*
        // bres_toys = ws->pdf( tag2 )->fitTo( *data_toys, RooFit::Strategy(2),
        // RooFit::Extended(kTRUE), RooFit::Save(kTRUE),
        // RooFit::Range("low,high") );
        if (f2 == "doubleExp") {
        ...
        bres_toys = ws->pdf(tag2)->fitTo(
            *data_toys, RooFit::Strategy(0), RooFit::Extended(kTRUE),
            RooFit::Save(kTRUE), RooFit::Range("Full"));
        bres_toys->SetName("bres_toys");

        fIntegral2 = ws->pdf(tag2)->createIntegral(mgg, RooFit::NormSet(mgg),
                                                   RooFit::Range("sig"));
        std::cout << "test Int2: " << fIntegral2->getVal() << std::endl;
        // std::cout << "signal events fit: " <<
        // (fIntegral2->getVal())*pow(ws->var("sideband_fit_singleExpse_Nbkg")->getVal(),2)
        // << std::endl;
        bias = (fIntegral2->getVal() - f1Integral->getVal()) /
               f1Integral->getVal();
    ...
    ws->pdf(tag2)->plotOn(f_mgg, RooFit::LineColor(kBlue),
                          RooFit::Range("Full"), RooFit::NormRange("Full"));
...
RooWorkspace *DoBiasTestSignal(TTree *tree, TString mggName, TString f1,
                               TString f2, int ntoys, double frac,
                               TString outName) {
    RooRandom::randomGenerator()->SetSeed(0);
    RooWorkspace *ws = new RooWorkspace("ws", "");
    ...
    double _Ns_hat, _Ns_sigma, _Ns, _alpha_hat, _alpha_sigma, _alpha, _Nbkg_hat,
        _Nbkg_sigma, _Nbkg;
    ...
    // double n_sideband = data.sumEntries("(mgg > 103. && mgg < 120.) || (mgg >
    // 135. && mgg < 160.)");
    TString sbCut = Form("(%s>%.2f && %s<%.2f) || (%s>%.2f && %s<%.2f)",
                         mggName.Data(), 103., mggName.Data(), 120.,
                         mggName.Data(), 135., mggName.Data(), 160.);
    ...
    //----------------------------------------------
    RooFitResult *bres =
        ws->pdf(tag1)->fitTo(data, RooFit::Strategy(2), RooFit::Extended(kTRUE),
                             RooFit::Save(kTRUE), RooFit::Range("low,high"));
    ...
    bFrame->SetTitle("");
    data.plotOn(bFrame);
    ws->pdf(tag1)->plotOn(bFrame, RooFit::LineColor(kBlue),
                          RooFit::Range("low,high"),
                          RooFit::NormRange("low,high"));
    ws->import(*bFrame);
    // saving firResult
    ...
    RooAbsReal *f1Integral_sb = ws->pdf(tag1)->createIntegral(
        mgg, RooFit::NormSet(mgg), RooFit::Range("low,high"));
    double f1Int_sb = f1Integral_sb->getVal();
    int npoints =
        (int)n_sideband / f1Int_sb; // re-scaling sideband to total bkg events
                                    // (N_sideband/NORMALIZE_INTEGRAL_SIDEBAND)
    npoints = 4 * npoints;
    ...
    RooAddPdf *sbModel = new RooAddPdf(
        "sbModel", "sbModel", RooArgList(*ws->pdf(tag2), *ws->pdf(gaussTag)),
        RooArgList(Nbkg, Ns));
    ws->import(*sbModel);
    ...
    RooFitResult *bres2p = ws->pdf(tag2p)->fitTo(
        *data_toys, RooFit::Save(kTRUE), RooFit::Strategy(2),
        RooFit::Extended(kTRUE), RooFit::Range("Full"));
    // RooFitResult* bres2p = ws->pdf( tag2p )->fitTo( data,
    // RooFit::Strategy(2), RooFit::Extended(kTRUE), RooFit::Save(kTRUE),
    // RooFit::Range("low,high") );
    ...
    //---------------------------------
    ws->pdf(tag2p)->plotOn(pFrame, RooFit::LineColor(kRed),
                           RooFit::Range("low,high"),
                           RooFit::NormRange("low,high"));
    ws->pdf(tag1)->plotOn(pFrame, RooFit::LineColor(kBlue),
                          RooFit::Range("low,high"),
                          RooFit::NormRange("low,high"));
    ws->import(*pFrame);

    //------------------------------------------------------------------------------
    // Define and obtain initial pdf parameters for f2, using sideband fit
    // parameters
    //------------------------------------------------------------------------------
    double dE_N1, dE_N2, dE_a1, dE_a2; // doubleExp
    double sE_N, sE_a;                 // singleExp
    ...
    for (int i = 0; i < ntoys; i++) {
        ...
        std::cout << "[INFO]: it-> " << i << " ======> stoys: " << stoysP
                  << " Nbkg: " << npointsP << " nBkG SR: " << f1Int * npoints
                  << std::endl;
        //-------------------------
        // Set Signal pdf paramaters
        //-------------------------
        Ns.setVal(stoysP); // Note initial paramenter is the true number of
                           // signal events
        ...
        //-------------------------------------------------------------
        // bres_toys = sbModel->fitTo( *data_toys, RooFit::Strategy(0),
        // RooFit::Extended(kTRUE), RooFit::Save(kTRUE), RooFit::Range("Full")
        // );
        ...
        // Defining Negative Log Likelihood (NLL)
        //--------------------------------------
        RooNLLVar *nll = (RooNLLVar *)sbModel->createNLL(
            *data_toys, RooFit::Extended(kTRUE), RooFit::Range("Full"));
   ...
   */
        ...
        /*
        if( !( _status == 0 && _covStatus == 3 && _status2 == 0 && _covStatus ==
        3 ) )
          {
            //_countPass++;
            //continue;
    ...
    sbModel->plotOn(ffPlot, RooFit::LineColor(kViolet), RooFit::Range("Full"),
                    RooFit::NormRange("low,high"));
    ws->import(*ffPlot);
    ...
    f_mgg->SetTitle("");
    data_toys->plotOn(f_mgg);
    // ws->pdf( tag2 )->plotOn( f_mgg, RooFit::LineColor(kViolet),
    // RooFit::Range("Full"), RooFit::NormRange("low,high"));

    // ws->pdf( tag2p )->plotOn( f_mgg, RooFit::LineColor(kBlue),
    // RooFit::Range("Full"), RooFit::NormRange("Full"),
    // RooFit::Normalization(npoints, RooAbsReal::NumEvent) ); ws->pdf( tag1
    // )->plotOn( f_mgg, RooFit::LineColor(kGreen), RooFit::Range("Full"),
    // RooFit::NormRange("Full"), RooFit::Normalization(npoints,
    // RooAbsReal::NumEvent) ); Plotting background only component of s+b fit
    sbModel->plotOn(f_mgg, RooFit::LineStyle(kDashed), RooFit::LineColor(kRed),
                    RooFit::Range("Full"), RooFit::NormRange("Full"),
                    RooFit::Components(tag2));
    sbModel->plotOn(f_mgg, RooFit::LineColor(kBlue), RooFit::Range("Full"),
                    RooFit::NormRange("Full"));
...
RooWorkspace *MakeSideBandFitAIC_2(TTree *tree, float forceSigma,
                                   bool constrainMu, float forceMu,
                                   TString mggName, double *Nbkg, double &AIC,
                                   double &AIC_2, double &AIC_3,
                                   double &fitStatus_1, double &fitStatus_2,
                                   double &fitStatus_3, double &fitStatus_4,
                                   TString ffName = "doubleExp") {
    RooWorkspace *ws = new RooWorkspace("ws", "");
    ...
    // Sideband Fit
    RooDataSet data("data", "", RooArgSet(mgg), RooFit::Import(*tree));
    double n = data.sumEntries(" (mGammaGamma>103 && mGammaGamma<120) || "
                               "(mGammaGamma>135 && mGammaGamma<160)");
    *Nbkg = n;
    ws->var("sideband_fit_" + ffName + "_Nbkg")->setVal(n * 1.2);

    // RooFitResult* bres = ws->pdf( tag )->fitTo( data, RooFit::Strategy(2),
    // RooFit::Extended(kTRUE), RooFit::Save(kTRUE), RooFit::Range("low,high")
    // );
    RooNLLVar *nll = (RooNLLVar *)ws->pdf(tag)->createNLL(
        data, RooFit::Extended(kTRUE), RooFit::Range("low,high"));
    ...
    // RooFit::Extended(kTRUE), RooFit::Range("low,high") ); RooAbsReal* nll =
    // ws->pdf( tag )->createNLL(data, RooFit::Extended(kTRUE),
    // RooFit::Range("Full") );
    std::cout << "nll_nll->" << nll->getVal() << std::endl;
    std::cout << "minNll->" << minNll << std::endl;
    RooArgSet *floatPars = ws->pdf(tag)->getParameters(data);
    double K = floatPars->getSize() - 1.;
    std::cout << "K -> " << K << std::endl;
    // double n = data.sumEntries(" (mgg>103 && mgg<120) || (mgg>135 &&
    // mgg<160)");
    ...
    if (n - K - 1 > 0) {
        ...
        std::cerr << "[ERROR]: (n-K-1) zero or negative!: " << (n - K - 1)
                  << "; setting AIC to 999 " << std::endl;
        AIC = 999.;
        AIC_2 = 999.;
        AIC_3 = 999.;
    }
    ...
    */
    ...
    RooPlot *fmgg = mgg.frame();
    RooDataSet *dataCut = (RooDataSet *)data.reduce(
        RooFit::Name("dataCut"), RooFit::SelectVars(RooArgSet(mgg)),
        RooFit::CutRange("low"));
    RooDataSet *dataHigh = (RooDataSet *)data.reduce(
        RooFit::Name("dataHigh"), RooFit::SelectVars(RooArgSet(mgg)),
        RooFit::CutRange("high"));
    ...
    */
    ...
    std::cout << ffName << " AIC fit status:  " << fitStatus_1 << ","
              << fitStatus_2 << "," << fitStatus_3 << "," << fitStatus_4
              << std::endl;
    return ws;
};

RooWorkspace *SelectBinning(TH1F *mggData, TString mggName, TString f1,
                            TString f2, int ntoys, int npoints,
                            TString outName) {
    ...
    TString totalEntriesStr = Form("(%s>%.2f && %s<%.2f)", mggName.Data(), 103.,
                                   mggName.Data(), 160.);
    TString sbEntriesStr = Form("(%s>%.2f && %s<%.2f) || (%s>%.2f && %s<%.2f)",
                                mggName.Data(), 103., mggName.Data(), 120.,
                                mggName.Data(), 135., mggName.Data(), 160.);
    ...
    RooAbsReal *fHistInt =
        histpdf.createIntegral(mgg, RooFit::NormSet(mgg), RooFit::Range("sig"));
    // mc toys dataHist
    ...
    TTree *biasTree =
        new TTree("biasTree", "Tree containing bias test information");
    ...
    RooFitResult *bres2 = ws->pdf(tag2)->fitTo(
        *data_toys2, RooFit::Strategy(2), RooFit::Extended(kTRUE),
        RooFit::Save(kTRUE), RooFit::Range("low,high"));
    RooAbsReal *fIntTrue = ws->pdf(tag2)->createIntegral(
        mgg, RooFit::NormSet(mgg), RooFit::Range("sig"));
    ...
    std::cout << "Number Of Elements: " << covMatrix.GetNoElements()
              << std::endl;
    ...
    */
    ...
    for (int i = 0; i < ntoys; i++) {
        ...
        /*
        std::cout << "===========================" << std::endl;
        std::cout << "[INFO]: Setting N_bkg-> " << npoints << " , ntrue: " <<
        n_true << "; and alpha-> " << alpha_clone << std::endl; std::cout <<
        "===========================" << std::endl;
        */
        /*
        ws->var( f1 + "_1_Nbkg" )->setVal( n_true );
        ws->var( f1 + "_1_a" )->setVal( alpha_clone );
        bres = ws->pdf( tag1 )->fitTo( *data_toys, RooFit::Strategy(0),
        RooFit::Extended(kTRUE), RooFit::Save(kTRUE), RooFit::Range("low,high")
        );
        */
        _status = -1;
        ...
        ws->var(f1 + "_1_Nbkg")->setVal(n_true);
        ws->var(f1 + "_1_a")->setVal(alpha_clone);
        // RooNLLVar* nll = (RooNLLVar*)ws->pdf( tag1 )->createNLL( *data_toys,
        // RooFit::Extended(kTRUE), RooFit::Range("low,high") );
        RooNLLVar *nll = (RooNLLVar *)ws->pdf(tag1)->createNLL(
            *data_toys, RooFit::Extended(kTRUE), RooFit::Range("Full"));
        RooMinimizer m(*nll);
        // m.fit("mhvr0");
        // ws->var( f1 + "_1_Nbkg" )->setVal( n_true );
        ...
        fIntegral2 = ws->pdf(tag1)->createIntegral(mgg, RooFit::NormSet(mgg),
                                                   RooFit::Range("sig"));
        RooAbsReal *f2int =
            ws->pdf(tag1)->createIntegral(mgg, RooFit::Range("sig"));
        RooAbsReal *f3int =
            ws->pdf(tag1)->createIntegral(mgg, RooFit::Range("Full"));
        RooAbsReal *f4int = ws->pdf(tag1)->createIntegral(
            mgg, RooFit::NormSet(mgg), RooFit::Range("Full"));
        ...
        // intErr     = myPdf->IntegralError( 103., 160., params,
        // covMatrix.GetMatrixArray(), 1e-14 ); intTF1_tot = myPdf->Integral(
        // 103., 160., 1e-15 );

        /*
          std::cout << "=======================" << i <<
        "==========================" << std::endl; std::cout << "Int RooFit
        Normalized --> " << fIntegral2->getVal() << std::endl; std::cout << "Int
        TF1 normalized --> " << intTF1/intTF1_tot << std::endl; std::cout <<
        "Int RooFit --> " << f2int->getVal() << std::endl; std::cout << "Int TF1
        --> " << intTF1 << std::endl; std::cout << "Int RooFit Total --> " <<
        f3int->getVal() << std::endl; std::cout << "Int RooFit Total Normalized
        --> " << f4int->getVal() << std::endl; std::cout << "Int TF1 Total --> "
        << intTF1_tot << std::endl; std::cout <<
        "=================================================" << std::endl;
        */
        // alpha
        ...
        double etaMod = 128.92 * exp(-alpha_hat * 128.92) -
                        122.08 * exp(-alpha_hat * 122.08);
        double xi = exp(-alpha_hat * 103.) - exp(-alpha_hat * 160.);
        double xiMod =
            103. * exp(-alpha_hat * 103.) - 160. * exp(-alpha_hat * 160.);
        double dIdN = eta / xi;
        double dIdAlpha = n_hat * (eta * xiMod / (xi * xi) + etaMod / xi);
        double sigmaI =
            sqrt(dIdN * dIdN * n_sigma * n_sigma +
                 dIdAlpha * dIdAlpha * alpha_sigma * alpha_sigma +
                 dIdN * dIdAlpha * pData[1] * n_sigma * alpha_sigma);

        // std::cout << "dIdN: " << dIdN << " , dIdAlpha: " << dIdAlpha <<
        // std::endl; std::cout << "Int: " << sigInt_hat << " +/- " << sigmaI <<
        // std::endl;
        sigInt_sigma = sigmaI;
        sigInt_sigma2 = sqrt(dIdN * dIdN * n_sigma * n_sigma +
                             dIdAlpha * dIdAlpha * alpha_sigma * alpha_sigma);
        biasTree->Fill();
    }
    //--------
    ...
    ws->pdf(tag2)->plotOn(fmgg, RooFit::LineColor(kBlue), RooFit::Range("Full"),
                          RooFit::NormRange("low,high"));
