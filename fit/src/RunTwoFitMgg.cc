...
RooWorkspace *DoubleCBFit(TTree *tree, TString mggName, float muCB,
                          float sigmaCB) {
    RooWorkspace *ws = new RooWorkspace("ws", "");
    RooRealVar mgg(mggName, "m_{#mu#mu}", SIDEBAND_LOW_BOTTOM_LIM, SIDEBAND_HIGH_UPPER_LIM, "GeV");
    mgg.setBins(38);
    mgg.setRange("Full", SIDEBAND_LOW_BOTTOM_LIM, SIDEBAND_HIGH_UPPER_LIM);
    // mgg.setRange("signal",110,140);

    RooRealVar w("event_weight", "w", 0, 100);
